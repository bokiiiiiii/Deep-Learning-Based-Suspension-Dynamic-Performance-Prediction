...
def perturbation_sensitivity_analysis(model, X_test, Y_test, perturbation=0.005):
    ...
    for i in range(num_inputs):
        ...
        sensitivity = np.mean(np.abs((perturbed_predictions - original_predictions) / original_predictions)*100, axis=0)
    ...
    sensitivity_df = pd.DataFrame(
        sensitivities,
        index   = ["ms", "mus", "iy", "cs", "ks", "kt", "h", "kratio", "cratio", "wr", "cg", "wb"],
        columns = ["sws1_max", "sws2_max", "a_rms", "dtl1_rms", "theta_rms", "dtheta_rms", "dtl2_rms", "SDPI"]
    )
...
if __name__ == "__main__":
    ...
    ann = tf.keras.models.load_model('two_axle_ann_model.h5', custom_objects={'PartialDense': PartialDense})
