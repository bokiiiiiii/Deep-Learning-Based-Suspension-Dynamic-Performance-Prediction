...
data_row = zeros(SIM_ITER_NUM, 34);
[MAX_origin, RMS_origin] = Three_Axle_Simulation_ISO8608(ms_origin, mus_origin, iy_origin, cs_origin, ks_origin, ...
...
for i = 1:SIM_ITER_NUM
    ...
    % Run the simulation
    [MAX, RMS] = Three_Axle_Simulation_ISO8608(ms, mus, iy, cs, ks, kt, h, kratio, cratio, wr, cg, wb, alpha);
    SDPI = 0.33 * (0.6 * RMS(2) / RMS_origin(2) + 0.2 * RMS(5) / RMS_origin(5) + 0.2 * RMS(6) / RMS_origin(6)) ...
        + 0.01 * (MAX(4) + MAX(8) + MAX(10)) / (MAX_origin(4) + MAX_origin(8) + MAX_origin(10)) ...
        + 0.66 * (RMS(3) + RMS(7) + RMS(9)) / (RMS_origin(3) + RMS_origin(7) + RMS_origin(9));
    
    if RMS(3) > 30000 || RMS(7) > 30000 || RMS(9) > 30000
...
filename = 'Three_Axle_Sim_Data.xlsx';
